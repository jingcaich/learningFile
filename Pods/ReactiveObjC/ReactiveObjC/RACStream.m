...
- (instancetype)setNameWithFormat:(NSString *)format, ... {
	...
	va_start(args, format);//va_start，函数名称，读取可变参数的过程其实就是在堆栈中，使用指针,遍历堆栈段中的参数列表,从低地址到高地址一个一个地把参数内容读出来的过程。

	NSString *str = [[NSString alloc] initWithFormat:format arguments:args];
	va_end(args);//宏va_arg()、va_start()和va_end()一起使用，便可以完成向函数传入数目可变的变元操作。取可变数目变元的典型例子是函数printf()。类型va_list是在<stdarg.h>中定义的。
